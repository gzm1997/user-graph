...
func main() {
	...
	fmt.Println(wr)
	mayKnowUserid := util.GetMayKnow(arr, wr)
	fmt.Println("最可能认识的人的userid", mayKnowUserid)
	mayKnowUser := util.LocateUser(mayKnowUserid, users)
	fmt.Println("最可能认识的人的名字", mayKnowUser)

	go func() {
		controllers.Content = "用户关系图谱"
		controllers.Users = users
		controllers.Files = files
		controllers.Groups = groups
		controllers.CreateGroupShareLinks = creategroupsharelinks
		controllers.ClickGroupShareLinks = clickgroupsharelinks
		controllers.CreateFileLinks = sfs
		controllers.ClickFileLinks = cfs
		controllers.Start = A
		controllers.MayKnows = mayKnowUser
		beego.Run()
	}()

	go func() {
		util.Open("http://localhost:8080")
	}()
	end := make(chan bool)
	<- end
}